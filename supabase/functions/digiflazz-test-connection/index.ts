...
// Pure JS MD5 — no deno.land/std dependency
  ...
  try {
    ...
    if (!username || !apiKey) {
      return new Response(JSON.stringify({
        success: false,
        message: "Username dan API Key Digiflazz belum dikonfigurasi. Isi di Pengaturan → Provider.",
        debug: debugInfo,
      }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    // Digiflazz signature untuk cek-saldo: MD5(username + apiKey + "depo")
    ...
    console.log(`[digiflazz-test] http=${res.status} duration=${duration}ms result=${rawText.substring(0, 300)}`);

    // Try to log (non-critical, ignore errors)
    try {
      await supabase.from("api_logs").insert({
        service: "digiflazz",
        endpoint: "/v1/cek-saldo",
        request_data: { username, sign_prefix: sign.substring(0, 8) },
    ...
    if ((result as { data?: { deposit?: number } }).data) {
      ...
      return new Response(JSON.stringify({
        success: true,
        balance,
        message: `Koneksi berhasil! Saldo: Rp ${Number(balance).toLocaleString("id-ID")}`,
        debug: debugInfo,
        response: result,
      }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const errMsg = (result as Record<string, string>).rd
      || (result as Record<string, string>).message
      || "Koneksi gagal — cek username dan API Key";
    ...
    console.error("[digiflazz-test] fatal error:", err);
    return new Response(JSON.stringify({
      success: false,
